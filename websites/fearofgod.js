...
exports.captchaWorker = function (request, task, profile) {
	...
	if (task['captchaHandler'] == 'manual') {
		...
		mainBot.requestCaptcha('fearofgod', task, false);
		const capHandler = () => {
		...
		if (task['captchaHandler'] == 'anticaptcha') {
			...
			request({
				...
				body: {
					...
					"task": {
						...
						"websiteURL": task['fearofgod']['referer'],
						"websiteKey": "6LdhYxYUAAAAAAcorjMQeKmZb6W48bqb0ZEDRPCl"
					}
				},
				json: true,
				agent: agent
			...
			request({
				url: 'https://2captcha.com/in.php?key=' + global.settings['2capAPIKey'] + '&method=userrecaptcha&googlekey=6LdhYxYUAAAAAAcorjMQeKmZb6W48bqb0ZEDRPCl&pageurl=' + task['fearofgod']['referer'] + '&json=1',
				method: 'GET',
				json: true,
				agent: agent
			}, function (error, response, body) {
...
exports.submitRaffle = function (request, task, profile) {
	...
	var form = JSON.parse(
		`{
			...
			"email_again": "",
			"${task['fearofgod']['sizeID']}": "${task['taskSizeSelect']}",
			"agree_to_rules": "yes",
	...
	request({
		...
		headers: {
			...
			'accept': '*/*',
			'referer': 'https://app.viralsweep.com/vrlswp/full/9571dc-58173?framed=1&ref=&hash=',
