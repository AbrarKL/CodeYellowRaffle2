...
exports.initTask = function (task, profile) {
	...
	if (profile['firstName'] == '' || profile['firstName'] == undefined || profile['firstName'] == null) {
		mainBot.mainBotWin.send('taskUpdate', {
			id: task.taskID,
			type: task.type,
			message: 'LSDLS requires a first name'
		});
	...
	if (profile['lastName'] == '' || profile['lastName'] == undefined || profile['lastName'] == null) {
		mainBot.mainBotWin.send('taskUpdate', {
			id: task.taskID,
			type: task.type,
			message: 'LSDLS requires a last name'
		});
		console.log(`[${task.taskID}] ` + JSON.stringify(profile));
		mainBot.taskStatuses[task['type']][task.taskID] = 'idle';
		return;
	...
	if (profile['phoneNumber'] == '' || profile['phoneNumber'] == undefined || profile['phoneNumber'] == null) {
		mainBot.mainBotWin.send('taskUpdate', {
			id: task.taskID,
			type: task.type,
			message: 'LSDLS requires a number (we reccomend to jig)'
		});
		console.log(`[${task.taskID}] ` + JSON.stringify(profile));
		mainBot.taskStatuses[task['type']][task.taskID] = 'idle';
		return;
	...
	request({
		url: task['variant'],
		headers: {
			'authority': 'sneakersmedia.us1.list-manage.com',
...
exports.postRaffleInfo = function (request, task, profile, token) {
	...
	request({
		url: 'https://lesitedelasneaker.us1.list-manage.com/subscribe/post',
		method: 'POST',
		headers: {
			'authority': 'lesitedelasneaker.us1.list-manage.com',
		...
		if (!error) {
			if (response.statusCode != 200 && response.request.href != 'https://lesitedelasneaker.us1.list-manage.com/subscribe/post') {
...
exports.captchaWorker = function (request, task, profile, token, mf) {
	...
	if (task['captchaHandler'] == 'manual') {
		...
		mainBot.requestCaptcha('lsdls', task, false);
		const capHandler = () => {
...
exports.submitRaffle = function (request, task, profile, token, mf) {
	...
	request({
		url: 'https://lesitedelasneaker.us1.list-manage.com/subscribe/confirm-captcha',
		method: 'POST',
		headers: {
			'authority': 'lesitedelasneaker.us1.list-manage.com',
			'cache-control': 'max-age=0',
			'origin': 'https://lesitedelasneaker.us1.list-manage.com',
			'upgrade-insecure-requests': '1',
			'content-type': 'application/x-www-form-urlencoded',
			'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.103 Safari/537.36',
			'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3',
			'referer': 'https://lesitedelasneaker.us1.list-manage.com/subscribe/post',
			'accept-language': 'en-GB,en-US;q=0.9,en;q=0.8'
		},
		formData: {
			'u': task['variant'].split('?u=')[1].split('&')[0],
		...
		if (!error) {
			...
			var error2 = $('.errorText').html();
			if (error2) {
				if (error2 == 'Captcha a échoué. Veuillez réessayer.') {
					mainBot.taskCaptchas[task['type']][task['taskID']] = '';
					mainBot.mainBotWin.send('taskUpdate', {
						id: task.taskID,
						type: task.type,
					...
					if (error2.toLowerCase().indexOf('est déjà abonné à la liste lsdls yeezy contest') !== -1) {
						mainBot.mainBotWin.send('taskUpdate', {
							id: task.taskID,
							type: task.type,
							message: error2
				...
				if (header.toLowerCase().indexOf('participation confirm') !== -1) {
					mainBot.mainBotWin.send('taskUpdate', {
						id: task.taskID,
						type: task.type,
