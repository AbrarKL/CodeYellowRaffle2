...
exports.performTask = function (task, profile) {
	...
	task['taskBirthDate'] = `${getRandomInt(1982, 2000)}-${getRandomInt(1, 9)}-${getRandomInt(1, 9)}`;


	mainBot.mainBotWin.send('taskUpdate', {
		id: task.taskID,
		type: task.type,
		message: 'Getting registration page (6S)'
	});
	console.log(`[${task.taskID}] ` + ' Getting registration page (6S)');
	request({
		...
		headers: {
			...
			'referer': 'https://www.oneblockdown.it/en/account',
		...
		if (!error && response.statusCode == 200) {
			mainBot.mainBotWin.send('taskUpdate', {
				id: task.taskID,
				type: task.type,
				message: 'Creating account (6S)'
			});
			console.log(`[${task.taskID}] ` + ' Creating account (6S)');
			request({
				url: 'https://www.oneblockdown.it/index.php',
				method: 'POST',
				headers: {
				...
				if (!error && response.statusCode == 200) {
					...
					if (parsed.success) {
						console.log(parsed);
						console.log('Account created. Press start again when email confirmed');
						mainBot.mainBotWin.send('taskUpdate', {
							id: task.taskID,
							type: task.type,
