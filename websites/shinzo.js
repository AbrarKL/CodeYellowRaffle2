...
exports.captchaWorker = function (request, task, profile, bb2) {
	...
	if (task['captchaHandler'] == 'manual') {
		...
		if (task['captchaHandler'] == 'anticaptcha') {
			...
			request({
				url: 'https://2captcha.com/in.php?key=' + global.settings['2capAPIKey'] + '&method=userrecaptcha&googlekey=6LczOjoUAAAAABEfbqdtD11pFD5cZ0n5nhz89nxI&pageurl=' + task["variant"] + '&json=1',
				method: 'GET',
				json: true,
				agent: agent
			}, function (error, response, body) {
