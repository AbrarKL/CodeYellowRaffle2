...
exports.captchaWorker = function (request, task, profile, viewkey) {
	...
	if (task['captchaHandler'] == 'manual') {
		...
		if (task['captchaHandler'] == 'anticaptcha') {
			...
			request({
				url: 'https://2captcha.com/in.php?key=' + global.settings['2capAPIKey'] + '&method=userrecaptcha&googlekey=6LetKEIUAAAAAPk-uUXqq9E82MG3e40OMt_74gjS&pageurl=' + task['dsmny']['mainLink'] + '&json=1',
				method: 'GET',
				json: true
			}, function (error, response, body) {
				if (error) {
